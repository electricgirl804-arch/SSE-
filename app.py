    ...
    for o in orders:
        with st.container(border=True):
            ...
            if col2.button("❌ رفض الطلب", key=f"no{o[0]}", use_container_width=True):
                c.execute("DELETE FROM orders WHERE id=?", (o[0],)); conn.commit(); st.warning("تم الرفض"); st.rerun()import streamlit as st, sqlite3, requests, math, datetime, json
import streamlit.components.v1 as components
import google.generativeai as genai
