...
st.title("🔧 إدارة الأحمال - IEC 60364")
...
def get_device_icon(name):
    ...
    elif "مروحة" in name: return "🌀"
    elif "تلفزيون" in name or "شاشة" in name: return "📺"
    elif "كمبيوتر" in name or "لابتوب" in name: return "💻"
    elif "غسالة" in name: return "🧺"
    elif "سخان" in name: return "🔥"
    elif "موتور" in name or "ماكينة" in name or "مضخة" in name: return "⚙️"
    elif "طابعة" in name: return "🖨️"
    elif "لحام" in name: return "🔩"
    elif "صاروخ" in name or "جلخ" in name: return "🪚"
    elif "كمبروسر" in name: return "💨"
    else: return "⚡"

if 'loads' not in st.session_state:
    st.session_state.loads = []

# قوالب جاهزة حسب IEC 60364
location_type = st.selectbox("نوع المكان", ["منزل سكني", "شقة/عمارة", "مؤسسة/مكتب", "مصنع صغير", "ورشة", "مزرعة", "محل تجاري"])
...
with col1:
    if st.button(f"📋 تحميل قالب {location_type}"):
        templates = {
            "منزل سكني": [
                ["لمبة LED",12,6,"مقاومي",0.95], 
                ["مروحة سقف",70,10,"حثي",0.85], 
                ["ثلاجة 14 قدم",150,24,"حثي",0.85], 
                ["مكيف 1.5 حصان",1200,8,"حثي",0.9],
                ["تلفزيون 43 بوصة",120,5,"إلكتروني",0.9]
            ],
            "ورشة": [
                ["ماكينة لحام",3000,3,"حثي",0.85], 
                ["صاروخ جلخ",2200,2,"حثي",0.85], 
                ["كمبروسر هواء",2200,4,"حثي",0.85], 
                ["لمبة ورشة",100,10,"مقاومي",0.95]
            ],
            "مصنع صغير": [
                ["موتور 3 فاز 5 حصان",3700,8,"حثي",0.88], 
                ["كمبروسر 2 حصان",1500,4,"حثي",0.85], 
                ["إضاءة LED",100,10,"مقاومي",0.95]
            ],
            "مزرعة": [
                ["موتور موية 2 حصان",1500,4,"حثي",0.85],
                ["لمبة حظيرة",50,12,"مقاومي",0.95],
                ["ثلاجة",200,24,"حثي",0.85]
            ]
        }
        if location_type in templates:
            ...
            for name, p, h, typ, eff in templates[location_type]:
                ...
                st.session_state.loads.append({
                    "الايقونة": icon, "الجهاز": name, "القدرة W": p, "الساعات": h, "النوع": typ,
                    "PF": pf, "الكفاءة": eff, "القدرة الفعلية W": actual_p,
                    "Surge W": surge, "الاستهلاك Wh": actual_p * h
                })
            st.success(f"تم تحميل قالب {location_type}")
            st.rerun()
with col2:
...
with st.expander("➕ إضافة جهاز جديد"):
    ...
    new_power = col2.number_input("القدرة W", 1, 50000, 100)
    new_hours = col3.number_input("ساعات/يوم", 0.5, 24.0, 5.0, 0.5)
    new_type = col4.selectbox("نوع الحمل", ["مقاومي", "حثي", "إلكتروني"])
    new_eff = col5.selectbox("الكفاءة", [0.98, 0.95, 0.9, 0.85, 0.8], format_func=lambda x: f"{x*100:.0f}%")
    ...
    if st.button("إضافة الجهاز"):
        if new_name:
            ...
            icon = get_device_icon(new_name)
            st.session_state.loads.append({
                "الايقونة": icon, "الجهاز": new_name, "القدرة W": new_power, "الساعات": new_hours, "النوع": new_type,
...
if st.session_state.loads:
    ...
    edited_df = st.data_editor(df, use_container_width=True, num_rows="dynamic", key="editor_images",
                               column_config={
                                   "الايقونة": st.column_config.TextColumn("الشكل", width="small"),
    ...
    if st.button("التالي → المحاكي 24 ساعة", use_container_width=True, type="primary"):
        st.switch_page("pages/03_⚡_المحاكي.py") # اتأكدي الاسم مطابق
else:
    st.info("اختاري نوع المكان واضغطي تحميل قالب أو أضيفي جهاز يدوي")
