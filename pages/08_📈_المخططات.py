...
.metric-card { background: rgba(255,255,255,0.95); padding: 20px; border-radius: 15px; 
               box-shadow: 0 8px 16px rgba(0,0,0,0.2); border-left: 5px solid #FFD700; }
...
if 'num_panels' in st.session_state:
    ...
    cost_batteries = st.session_state.num_batteries * price_battery
    ...
    with col2:
        st.markdown(f"<div class='metric-card'><h4>عدد البطاريات</h4><h2>{st.session_state.num_batteries}</h2></div>", unsafe_allow_html=True)
    with col3:
        st.markdown(f"<div class='metric-card'><h4>قدرة الانفرتر</h4><h2>{st.session_state.inverter_kw} kW</h2></div>", unsafe_allow_html=True)
    with col4:
    ...
    saving_yearly = st.session_state.energy_kwh * 365 * 0.15
    with col1:
        st.metric("التكلفة الكلية", f"${total:,.0f}")
        st.metric("الطاقة اليومية", f"{st.session_state.energy_kwh:.1f} kWh")
    with col2:
        st.metric("فترة الاسترجاع", f"{st.session_state.payback:.1f} سنة")
        st.metric("التوفير السنوي", f"${saving_yearly:,.0f}")
    st.markdown("</div>", unsafe_allow_html=True)

else:
    st.warning("⚠️ ارجع للصفحة الرئيسية لإدخال بيانات الأحمال أولاً")
